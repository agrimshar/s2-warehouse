...
def main() -> None:
    ...
    spark = build_spark()
    with timed("silver_compute", start=args.start, end=args.end) as m:
        ...
        if not args.from_staging:
            ...
            m["scenes"] = len(scenes)
            m["rows_out"] = spark.read.parquet(str(STAGING)).count()
        ...
        ops = DeltaTable.forPath(spark, args.out).history(1).first()["operationMetrics"]
